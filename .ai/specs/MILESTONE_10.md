...
## Phase 3: Per-PE Routing Tables + Schedule IR Route Refactor
...
Additionally, single-route tasks (`ForwardActivationEntry`, `LinearEntry`, `RmsNormPartialSumEntry`) use ad-hoc inline fields (`route_dest`/`route_hops`/`payload_slot`/`route_color` or `reduce_*` variants) instead of `BroadcastRoute` objects. This forces every pass that touches routes (color, lower) to branch on task type. Since Phase 3 is already removing hop fields from the artifact and Rust side, this is the natural time to unify the Python schedule IR: all routed tasks use `routes: list[BroadcastRoute]`.

### Schedule IR route refactor (Python side)

Unify single-route tasks to use `routes: list[BroadcastRoute]` (with exactly 1 element):

**`schedule_ir.py`**:
- `ForwardActivationEntry`: remove `route_dest`, `route_hops`, `payload_slot`, `route_color`. Add `routes: list[BroadcastRoute]`.
- `LinearEntry`: remove `route_dest`, `route_hops`, `fragment_slot`, `route_color`. Add `routes: list[BroadcastRoute]`. Keep `fragment_offset` (task config, not routing).
- `RmsNormPartialSumEntry`: remove `reduce_dest`, `reduce_hops`, `partial_sum_slot`, `route_color`. Add `routes: list[BroadcastRoute]`.

The mapping is: `route_dest`/`reduce_dest` → `BroadcastRoute.dest`, `route_hops`/`reduce_hops` → `BroadcastRoute.hops`, `payload_slot`/`fragment_slot`/`partial_sum_slot` → `BroadcastRoute.payload_slot`, `route_color` → `BroadcastRoute.color`.

**`route.py`**: Where these entries are constructed, build a `BroadcastRoute` instead of setting inline fields.

**`color.py`**: The `_enumerate_routes` function simplifies — all routed tasks have `routes: list[BroadcastRoute]`, no more isinstance branching for single-route tasks. `_write_back_colors` also simplifies.

### Routing table model (Rust side)
...
- **`lower.py`**: Mechanically copies `PESchedule.routing_table` into `PEProgram.routing_table`. Strips `hops` and `deliver_at` from `BroadcastRouteTask` (no longer needed — routing is in the tables). Strips `route_hops`/`reduce_hops` from single-route tasks (they now use `routes` list with `BroadcastRoute` objects).
- **`color.py`**: Simplified route enumeration (all tasks use `routes` list).
...
Single-route artifact tasks (`ForwardActivationTask`, `LinearTask`, `RmsNormPartialSumTask`) lose their `route_hops`/`reduce_hops` fields and gain `routes: list[BroadcastRouteTask]`. They keep `route_dest`/`reduce_dest` only if needed for profiling, otherwise remove.
...
**`pe.rs`** — `PE` gains `routing_table: HashMap<u32, RouteAction>`. `BroadcastRouteRuntime` simplifies (loses `hops`, `deliver_at`). `TaskKind` variants for single-route tasks (`ForwardActivation`, `Linear`, `RmsNormPartialSum`) switch from inline hop fields to `routes: Vec<BroadcastRouteRuntime>`.

**`program.rs`** — `PEProgram` serde struct gains `routing_table`. `BroadcastRouteProgram` loses `hops`, `deliver_at`. Single-route `TaskProgram` variants switch to `routes` list. `convert_task` and `convert_routes` simplified.
...
Single-route task execution arms (`ForwardActivation`, `Linear`, `RmsNormPartialSum`) switch from inline `emit_message(... hops ...)` to reading from `routes[0]` and calling `broadcast_to_dests` or `emit_message` with just the color.

### Test/injection backward compatibility

- **`inject_message`**: Self-delivery (source == dest). No routing table entry needed. Remove `hops`/`current_hop`/`deliver_at` from `Message` literal.
- **`add_task`** (`InjectTaskKind::ForwardActivation`): Rewrite to compute routing table entries on intermediate PEs instead of storing hops in `TaskKind`. The caller provides a color, `add_task` populates routing table entries along the path.
- **`bridge.rs`**: Uses `inject_message` (self-delivery, unchanged). Update if it uses `add_task`.

### Validation

The lowering pass validates that no PE has conflicting routing table entries. With the intermediate-PE-overlap conflict graph from Phase 1, this should never happen — but the validation catches bugs.

### Files changed

| File | Change |
|------|--------|
| `schedule_ir.py` | Unify single-route tasks to use `routes: list[BroadcastRoute]`; remove inline route fields |
| `route.py` | Construct `BroadcastRoute` objects for single-route tasks |
| `color.py` | Simplify `_enumerate_routes` and `_write_back_colors` — all tasks use `routes` |
| `lower.py` | Copy `routing_table`; strip hop fields; handle unified route format |
| `artifact.py` | Add `RouteTableEntry`, `routing_table` on `PEProgram`; simplify `BroadcastRouteTask`; unify single-route tasks |
| `message.rs` | Remove `hops`, `current_hop`, `deliver_at`; add `hop_count` |
| `pe.rs` | Add `RouteAction`, `routing_table` on `PE`; simplify `BroadcastRouteRuntime`; unify `TaskKind` route fields |
| `program.rs` | Add routing table serde; simplify route/task serde structs |
| `runtime.rs` | Table-based `process_deliver`; update `emit_message`/`emit_broadcast_message`; rewrite `add_task`; unify single-route task execution |
| `bridge.rs` | Update `add_task` usage if applicable |
| Tests | Update all tests constructing route structures |
